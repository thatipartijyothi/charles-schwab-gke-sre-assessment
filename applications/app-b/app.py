...
class JsonFormatter(logging.Formatter):
    def format(self, record):
        return json.dumps({"severity": record.levelname, "message": record.getMessage(), "app": APP_NAME,
                           "cluster": CLUSTER_NAME, "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())})
...
@app.get("/")
def home():
    return jsonify(application=APP_NAME, cluster=CLUSTER_NAME, status="ok", message="Welcome to Application B")
...
@app.get("/work")
def work():
    delay_ms = min(int(request.args.get("delay_ms", 100)), 2000)
    with LATENCY.labels(APP_NAME, "/work").time():
        time.sleep(delay_ms / 1000)
    return jsonify(delay_ms=delay_ms, processed_by=APP_NAME)
